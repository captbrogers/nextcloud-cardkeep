	...
	<div id="content" class="app-cardkeep">
		<AppNavigation>
			<AppNavigationNew v-if="!loading"
				:text="t('cardkeep', 'New note')"
				:disabled="false"
				button-id="new-cardkeep-button"
				button-class="icon-add"
				@click="newNote" />
			<ul>
				<AppNavigationItem v-for="note in notes"
					:key="note.id"
					:title="note.title ? note.title : t('cardkeep', 'New note')"
					...
					<template slot="actions">
						<ActionButton v-if="note.id === -1"
							...
							{{
							t('cardkeep', 'Cancel note creation') }}
						</ActionButton>
						<ActionButton v-else
							icon="icon-delete"
							@click="deleteNote(note)">
							{{
							 t('cardkeep', 'Delete note') }}
						</ActionButton>
					</template>
				</AppNavigationItem>
			</ul>
		...
		<AppContent>
			...
			<div v-else id="emptycontent">
				<div class="icon-file" />
				<h2>{{
				 t('cardkeep', 'Create a note to get started') }}</h2>
			</div>
		</AppContent>
	</div>
</template>
